...
class GetAllMatchesOfTeamUseCaseTest {
    @Mock
    ...
    fun getAllMatchesOfTeamThenSuccess() = runTest {
        ...
        given(matchDataSource.getMatchesOfTeam("1")).willReturn(flow {
            emit(givenMatches)
        })

        val useCase = GetAllMatchesOfTeamUseCase(matchDataSource)
        useCase("1").collect { resultMatches ->
            assertTrue(
